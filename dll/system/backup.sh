...
MYIP=$(curl -sS ipv4.icanhazip.com)
NameUser=$(curl -sS https://raw.githubusercontent.com/kulo-sinten/mpek/main/REG/ipvps | grep $MYIP | awk '{print $2}')
...
LLatest=`date`
Get_Data () {
git clone https://github.com/scvpn/user-backup-db.git /root/user-backup/ &> /dev/null
}
...
Save_And_Exit () {
    ...
    git push -f https://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@github.com/scvpn/user-backup-db.git &> /dev/null
}
...
link="https://raw.githubusercontent.com/scvpn/user-backup-db/main/$NameUser/$NameUser.zip"
sleep 1
